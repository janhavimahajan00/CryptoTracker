...
public class CurrencyRVAdapter extends RecyclerView.Adapter<CurrencyRVAdapter.ViewHolder> {
    ...
    public void onBindViewHolder(@NonNull CurrencyRVAdapter.ViewHolder holder, int position) {
     CurrencyRVModal currrencyRVModal=currencyRVModalArrayList.get(position);
             holder.currencyNameTV.setText(currrencyRVModal.getName());
             holder.symbolTV.setText(currrencyRVModal.getSymbol());
             holder.rateTV.setText("$"+df2.format(currrencyRVModal.getPrice()));
    }
